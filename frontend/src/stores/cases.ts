...
export const useCaseStore = defineStore('cases', () => {
  const items = ref<LocalizationCase[]>([]); const loading = ref(false); const total = ref(0); const batchResult = ref<BatchAnalyzeResponse | null>(null); const batchRunning = ref(false)
  ...
  async function runBatch(body: object): Promise<BatchAnalyzeResponse> { batchRunning.value = true; try { const { data } = await caseApi.batchAnalyze(body); batchResult.value = data.data; return data.data } finally { batchRunning.value = false } }
  function clearBatch() { batchResult.value = null }
  ...
  return { items, loading, total, batchResult, batchRunning, fetch, create, analyze, runBatch, clearBatch, confirm, close }
})
