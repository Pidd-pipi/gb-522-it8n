...
export const useCaseStore = defineStore('cases', () => {
  ...
  async function batchAnalyze(body: object): Promise<BatchAnalyzeResponse> { const { data } = await caseApi.batchAnalyze(body); for (const result of data.data.results) { if (result.case) replace(result.case) } return data.data }
  ...
  return { items, loading, total, fetch, create, analyze, batchAnalyze, confirm, close }
})
